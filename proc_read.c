...
static size_t populate_proc_info(char args[5][16]) {
char * target = args[0];
bool nflg = false;
...
for_each_process(task) {
char poid[16];
scnprintf(poid, 16, "%d",task->pid);
if (strcmp(poid,target) == 0) {
    const struct cred* ids = task->cred; 
    unsigned int state = task->__state;
    char other_state[10];
    scnprintf(other_state,10,"0x%x",state);
...
static ssize_t my_write(struct file *fd, const char __user *target, size_t len,loff_t *off) {
char query[256];
if (copy_from_user(query, target, 256)) {
 return -EFAULT;
}
...
if (arg_c > 5) {
  printk(KERN_ALERT "ARGS can't exceed a maximum of 5");
  return 0;
}
args[arg_c][cur++]=*i;
i++;
...
/* 
Main sub service choose and passing args to that sub service
*/
if (strcmp(cmd,"proc") == 0){
  if (populate_proc_info(args) != 0) {
    printk(KERN_WARNING "Some Error might've occured!");
    return -EFAULT;
  }
}else if (strcmp(cmd,"cpu") == 0){
  if (populate_cpu_info(args) != 0) {
    printk(KERN_WARNING "Some Error might've occured!");
    return -EFAULT;
  }
}else{
